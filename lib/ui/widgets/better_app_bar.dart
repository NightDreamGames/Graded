...
/// app bar, see [BetterSliverAppBar], which embeds an [BetterAppBar] in a sliver for use in
/// a [CustomScrollView].
///
...
///
/// If the [leading] widget is omitted, but the [BetterAppBar] is in a [Scaffold] with
...
/// stretching to start.
/// ///
/// The [BetterAppBar] insets its content based on the ambient [MediaQuery]'s padding,
/// to avoid system UI intrusions. It's taken care of by [Scaffold] when used in
/// the [Scaffold.appBar] property. When animating an [BetterAppBar], unexpected
/// [MediaQuery] changes (as is common in [Hero] animations) may cause the content
/// to suddenly jump. Wrap the [BetterAppBar] in a [MediaQuery] widget, and adjust its
/// padding such that the animation is smooth.
///
/// {@tool dartpad}
/// This sample shows an [BetterAppBar] with two simple actions. The first action
...
/// {@tool dartpad}
/// This sample shows the creation of an [BetterAppBar] widget with the [shadowColor] and
...
///
/// {@tool dartpad}
/// This sample shows an [BetterAppBar] with two action buttons with their primary
...
/// See also:
///
///  * [Scaffold], which displays the [BetterAppBar] in its [Scaffold.appBar] slot.
///  * [BetterSliverAppBar], which uses [BetterAppBar] to provide a flexible app bar that
///    can be used in a [CustomScrollView].
///  * [TabBar], which is typically placed in the [bottom] slot of the [BetterAppBar]
...
class BetterAppBar extends StatefulWidget implements PreferredSizeWidget {
  ...
  /// Used by [Scaffold] to compute its [BetterAppBar]'s overall height. The returned value is
  /// the same `preferredSize.height` unless [BetterAppBar.toolbarHeight] was null and
  ...
  /// the app bar's [BetterAppBar.bottom] widget.
  ...
  /// [BetterAppBar] will imply an appropriate widget. For example, if the [BetterAppBar] is
  /// in a [Scaffold] that also has a [Drawer], the [Scaffold] will fill this
  /// widget with an [IconButton] that opens the drawer (using [Icons.menu]). If
  /// there's no [Drawer] and the parent [Navigator] can go back, the [BetterAppBar]
  ...
  /// See also:
  ///
  ///  * [Scaffold.appBar], in which an [BetterAppBar] is usually placed.
  ...
  ///
  /// A flexible space isn't actually flexible unless the [BetterAppBar]'s container
  /// changes the [BetterAppBar]'s size. A [BetterSliverAppBar] in a [CustomScrollView]
  /// changes the [BetterAppBar]'s height when scrolled.
  ///
  ...
  /// null:
  /// In Material v2 (i.e., when [ThemeData.useMaterial3] is false),
  /// then [BetterAppBar] uses the overall theme's [ColorScheme.primary] if the
  /// overall theme's brightness is [Brightness.light], and [ColorScheme.surface]
  /// if the overall theme's brightness is [Brightness.dark].
  /// In Material v3 (i.e., when [ThemeData.useMaterial3] is true),
  /// then [BetterAppBar] uses the overall theme's [ColorScheme.surface]
  ///
  ...
  /// value is also null:
  /// In Material v2 (i.e., when [ThemeData.useMaterial3] is false),
  /// then [BetterAppBar] uses the overall theme's [ColorScheme.onPrimary] if the
  /// overall theme's brightness is [Brightness.light], and [ColorScheme.onSurface]
  /// if the overall theme's brightness is [Brightness.dark].
  /// In Material v3 (i.e., when [ThemeData.useMaterial3] is true),
  /// then [BetterAppBar] uses the overall theme's [ColorScheme.onSurface].
  ///
  ...
  /// used by [BetterSliverAppBar] to animate the opacity of the toolbar when the app
  /// bar is scrolled.
  /// {@endtemplate}
  ...
  /// Defines the height of the toolbar component of an [BetterAppBar].
  ///
  ...
  /// Defines the width of [BetterAppBar.leading] widget.
  ///
  /// By default, the value of [BetterAppBar.leadingWidth] is 56.0.
  /// {@endtemplate}
  ...
  @override
  State<BetterAppBar> createState() => _BetterAppBarState();
}

class _BetterAppBarState extends State<BetterAppBar> {
...
/// the screen see [BetterAppBar], which is used in the [Scaffold.appBar] slot.
///
...
///
/// {@tool dartpad}
/// Here is an example of [BetterSliverAppBar] when using [stretch] and [onStretchTrigger].
///
...
///
/// {@tool dartpad}
/// This sample shows a [BetterSliverAppBar] and it's behavior when using the
...
/// See also:
///
///  * [CustomScrollView], which integrates the [BetterSliverAppBar] into its
///    scrolling.
///  * [BetterAppBar], which is a fixed-height app bar for use in [Scaffold.appBar].
///  * [TabBar], which is typically placed in the [bottom] slot of the [BetterAppBar]
///    if the screen has multiple pages arranged in tabs.
///  * [IconButton], which is used with [actions] to show buttons on the app bar.
///  * [PopupMenuButton], to show a popup menu on the app bar, via [actions].
...
class BetterSliverAppBar extends StatefulWidget {
  ...
  ///
  /// Returns a [BetterSliverAppBar] configured with appropriate defaults
  ...
  /// See also:
  ///
  ///   * [BetterAppBar], for a small or center-aligned top app bar.
  ...
  const BetterSliverAppBar.medium({
    super.key,
    this.leading,
    this.automaticallyImplyLeading = true,
    this.title,
  ...
  ///   * https://m3.material.io/components/top-app-bar/overview, the Material 3
  ///     app bar specification.
  const BetterSliverAppBar.large({
    super.key,
    this.leading,
    this.automaticallyImplyLeading = true,
    this.title,
  ...
  /// content is not scrolled under the [BetterAppBar].
  ///
  /// Defaults to false, meaning that the [elevation] is only applied when the
  /// [BetterAppBar] is being displayed over content that is scrolled under it.
  ///
  ...
  /// See also:
  ///
  ///  * [BetterSliverAppBar] for more animated examples of how this property changes the
  ...
  @override
  State<BetterSliverAppBar> createState() => _BetterSliverAppBarState();
}

// This class is only Stateful because it owns the TickerProvider used
// by the floating appbar snap animation (via FloatingHeaderSnapConfiguration).
class _BetterSliverAppBarState extends State<BetterSliverAppBar> with TickerProviderStateMixin {
  ...
  @override
  void didUpdateWidget(BetterSliverAppBar oldWidget) {
  ...
  Widget build(BuildContext context) {
    ...
    switch (widget._variant) {
      ...
      case _SliverAppVariant.medium:
        effectiveExpandedHeight = widget.expandedHeight ?? _MediumScrollUnderFlexibleConfig.expandedHeight + bottomHeight;
        effectiveCollapsedHeight = widget.collapsedHeight ?? topPadding + _MediumScrollUnderFlexibleConfig.collapsedHeight + bottomHeight;
        effectiveFlexibleSpace = widget.flexibleSpace ??
      ...
      case _SliverAppVariant.large:
        effectiveExpandedHeight = widget.expandedHeight ?? _LargeScrollUnderFlexibleConfig.expandedHeight + bottomHeight;
        effectiveCollapsedHeight = widget.collapsedHeight ?? topPadding + _LargeScrollUnderFlexibleConfig.collapsedHeight + bottomHeight;
        effectiveFlexibleSpace = widget.flexibleSpace ??
            _ScrollUnderFlexibleSpace(
              title: widget.title,
              foregroundColor: widget.foregroundColor,
